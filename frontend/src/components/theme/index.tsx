...
const Theme: React.FC<ThemeProps> = ({
    ...
    useEffect(() => {
        ...
        const fetchVideos = async () => {
            ...
            try {
                ...
                const result:any = themes
                    ? await getRequest(`video/themes/${themes}`, setLoading)
    ...
    const filterVideos = (themes?: string) => {
        return videos.filter((v: any) =>
            themes ? v.rating.includes(themes) : v
        );
    };
    ...
    return (
        ...
        >
            <div className="mt-1 w-full sm:px-4 py-2 flex justify-between items-center">
                ...
                {filterVideos(themes).length > 6 && title && (
                    <a
                        href="#"
                        className={` ${
                            !title && "float-right"
            ...
            <div className="flex flex-wrap w-full">
                {filterVideos(themes).map((video: any, index: any) => (
                    <div
                        key={index}
                        style={{ maxWidth: "20%" }}
                        className="cursor-pointer gallery relative flex-wrap  border-gray-200  w-full   p-2 "
